...
- S1 temporal average and standard deviations for HH, HV and HH-HV over one year
- S2 Bands (10m and 20m superresolved to 10m), derivative indices, and cloud
  masks from fmask
...
satellite_layers = np.concatenate((s1vhA_summer.values, s1vvA_summer.values, s1diffA_summer.values,
                            ...
                            s1vhD_std_summer.values, s1vvD_std_summer.values, s1diffD_std_summer.values),axis=0)
...
rf_no_trees = RandomForestClassifier()
...
for kk in range(0,k):
    ...
    rf.fit(X[cal_blocks!=kk],y[cal_blocks!=kk])
    y_rf[val_blocks==kk] = rf.predict(X[val_blocks==kk])

y_rf_simple[y_rf<2.5]=1
y_rf_simple[y_rf>2.5]=2
...
fig1.show()
fig1.tight_layout()
fig1.savefig('ConfusionMatrix_RF_%s_s1_only.png' % site)
...
fig2.show()
fig2.savefig('landcover_classifications_%s_s1_only.png' % site)
